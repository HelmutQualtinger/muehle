...
Mühle (Nine Men's Morris): a Flask backend serving a vanilla HTML/CSS/JS frontend. The board is rendered as inline SVG and driven entirely through a small JSON API — there is no client-side game logic beyond rendering and input handling; all rules are enforced server-side.
...
**`static/js/game.js`** — single IIFE, no framework, no build step (edit and refresh). Key pieces:
- Renders the board as SVG built once (`buildBoardSkeleton`) then updated in place (`render`) by diffing `state.board` against tracked DOM elements (`stoneEls`).
- Point (empty-square) hit targets are drawn as two overlaid circles — a small visible dot plus a larger invisible one (`pointer-events` split via `style.pointerEvents = "none"` on the visible one) — because the visible dots are the smallest targets on the board and are easy to click-miss otherwise.
- `processResult()` replays a response's `events` array with a short delay between steps (`EVENT_DELAY_MS`), driving both the mill-flash SVG animation and the synthesized Web Audio sound effects (`Sound` object — everything is generated tones, no audio assets).
...
**`templates/index.html`** — server-rendered per route: `game_id` is `None` on `/` (renders the local-play settings UI) and set on `/g/<id>` (renders the network invite panel instead). `window.MUEHLE_GAME_ID` is how `game.js` learns which mode it's in.
