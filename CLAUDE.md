...
- `/` and `/g/<id>` also pass Open Graph/Twitter preview context (`page_title`, `page_description`, `page_url`, `preview_image_url`, built from `_preview_context()`) into `templates/index.html`, and register `asset_version(filename)` as a Jinja global — an MD5 of the file's contents, cached in `_ASSET_VERSIONS` — appended as a `?v=` query string to `style.css`/`game.js` so a reverse proxy's long `Cache-Control` on `/static/*` can't serve a stale asset after a deploy.
...
- The `Music` object is a separate always-on-request Web Audio loop (a synthesized arrangement of the public-domain "Also sprach Zarathustra" fanfare, not a sample), independent of `Sound`'s one-shot effects; its on/off state persists in `localStorage` (`muehle_music`) and, like `Sound`, only actually starts on the first `pointerdown` (browsers block autoplay before a user gesture).
- Each point has a ring-shaped `auraMeshes[i]` toggled in `render()`: red (`AURA_RED`) on a stone that's currently a legal capture target during a pending removal, green (`AURA_GREEN`) on the point that was placed/moved-to last, so a player can see what just happened without replaying the event log.
- Two easter eggs live in the scene graph alongside the board: a small stylized starship (`spaceshipGroup`, built in `makeSpaceship()`) that periodically drifts past in `animate()`, and a hidden note painted into the board's underside texture (`makeBoardUndersideTexture()`) — invisible in normal play since the camera never looks up from below, findable by orbiting the board upside down.
